...
class AnimatedSprite(Sprite):
    ...
    def animate(self, dt):
        self.frame_index += self.animation_speed * dt   # DELTA TIME fairly important to be sure that animation is same on every computer
        self.image = self.frames[int(self.frame_index % len(self.frames))] # With % frame index will never exceed the lenght of self.frames
...
class ParticleEffectSprite(AnimatedSprite):
    def __init__(self, pos, frames, groups):
        super().__init__(pos, frames, groups)
        self.rect.center = pos
        self.z = Z_LAYERS['fg'] # On top of everythin else
