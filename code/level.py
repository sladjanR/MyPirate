...
class Level:
    def __init__(self, tmx_map, level_frames, data):
        ...
        self.collision_sprites = pygame.sprite.Group()  # This group contains all collidable sprites
    ...
    def setup(self, tmx_map, level_frames):
        ...
        # x, y - is not pixel positions, it is positions in a grid, because of that we have * TILE_SIZE
            for x, y, surf in tmx_map.get_layer_by_name(layer).tiles():
                ...
                Sprite((x * TILE_SIZE,y * TILE_SIZE), surf, groups, z)     # We have tuple of sprites
            # print(tmx_map)
        ...
        for obj in tmx_map.get_layer_by_name('Objects'):
            ...
            if obj.name == "player":
                ...
                if obj.name in ('barrel','crate'):
                    ...
                    #TODO take a notice here, these are animated objects
                    #frames
                    frames = level_frames[obj.name] if not 'palm' in obj.name else level_frames['palms'][obj.name]
                    if obj.name == 'floor_spike' and obj.properties['inverted']:
                        frames = [pygame.transform.flip(frame, False, True) for frame in frames]    # False horizontal flip, True vertical flip
        ...
        for obj in tmx_map.get_layer_by_name("Moving Objects"):
            ...
            if obj.name == 'spike':
                ...
                if obj.width > obj.height: # Horizontal movement
                    move_dir = "x"
                    start_pos = (obj.x, obj.y + obj.height / 2)     # In tiled it is on the middle on top so we divide by 2
        ...
        for obj in tmx_map.get_layer_by_name('Water'):
            ...
            for row in range(rows):
                for col in range(cols):
                    x = obj.x + col * TILE_SIZE     # With that we have positions of all of the tiles
                    y = obj.y + row * TILE_SIZE
                    if row == 0:                    # Only for first row we want to have animation on the water
                        AnimatedSprite((x,y), level_frames['water_top'], self.all_sprites, Z_LAYERS['water'])  # Animated sprite
                    else:
                        # Regular sprite
